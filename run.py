...
class Runner(object):
    ...
    def __init__(self, params):
        ...
        if self.p.gpu != '-1' and torch.cuda.is_available():
            ...
            torch.cuda.set_rng_state(torch.cuda.get_rng_state())  #随机数
    ...
    def load_data(self):
        ent_set, rel_set = OrderedSet(), OrderedSet()  #set of entities and relations
        ...
        self.rel2id.update({rel + '_reverse': idx + len(self.rel2id) for idx, rel in enumerate(rel_set)})   #add reverse relations
        ...
        self.data = ddict(list)  # set of all triples
        sr2o = ddict(set)  # temp structure for storing mapping
        for split in ['train', 'test', 'valid']:
            for line in open('./data/{}/{}.txt'.format(self.p.dataset, split)):
                sub, rel, obj = map(str.lower, line.strip().split('\t'))
        ...
        self.sr2o_train = {k : list(v) for k, v in sr2o.items()}
        ...
        # all triples not in dataset are regarded as negtive 
        # all triples with same sub and rel will be stored in one training data
        if self.p.strategy == 'one_to_n':
            for (sub, rel), obj in self.sr2o_train.items():
                self.triples['train'].append({'triple' : (sub, rel, -1), 'label': self.sr2o_train[(sub, rel)]})
                
        # random choose some triples which doesn't apper in dataset to create negtive samples
        # one triple equal one training data
        elif self.p.strategy == 'one_to_x':
            for sub, rel, obj in self.data['train']:
                rel_inv = rel + self.p.num_rel
                self.triples['train'].append({'triple' : (sub, rel, obj), 'label' : self.sr2o_train[(sub, rel)]})
                self.triples['train'].append({'triple' : (obj, rel_inv, sub), 'label' : self.sr2o_train[(obj, rel_inv)]})
    ...
    def add_model(self, model, score_func):
        ...
        if model_name.lower() == 'hran_transe':
            model = HRAN_TransE(self.edge_index, self.edge_type, params=self.p)
        elif model_name.lower() == 'hran_distmult':
            model = HRAN_DistMult(self.edge_index, self.edge_type, params=self.p)
        elif model_name.lower() == 'hran_conve':
            model = HRAN_ConvE(self.edge_index, self.edge_type, params=self.p)
        elif model_name.lower() == 'hran_convd':
            model = HRAN_ConvD(self.edge_index, self.edge_type, params=self.p)
        elif model_name.lower() == 'hran_conv_transe':
            model = HRAN_Conv_TransE(self.edge_index, self.edge_type, params=self.p)
        else:
    ...
    def add_optimizer(self, model):
        ...
        return torch.optim.Adam(model.parameters(), lr=self.p.lr, weight_decay=self.p.l2)
    ...
    def predict(self, split='valid', mode='tail_batch'):
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(train_iter):
                sub, rel, obj, label, _ = self.read_batch(batch, split)  #obj : [batch]
                pred = self.model.forward(sub, rel, None)   # [batch, lable.shape[1]]  
                b_range = torch.arange(pred.size()[0], device=self.device)  #[0, 1, ..., batch-1]
                target_pred = pred[b_range, obj]   #[batch]  pred value of correct label
                # filter setting
...
def main(model):
    try:
        model.fit()
        model.logger.info('train success')
        #sys.exit(0)
    except Exception as e:
        model.logger.info('training failed')
        print(e)
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-model', dest='model', default='hran', help='Model Name')
